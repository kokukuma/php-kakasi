...
//
//	// EUCに変換
//	convert("UTF-8","EUC-JP",word->value.str.val,buf, MYBUFSZ);
//
//	// kakasiのオプションを設定
//	ret = kakasi_getopt_argv(2, option);
//
...
//		// kakasiを実行
//		res = kakasi_do(&buf);
//		convert("EUC-JP","UTF-8",res,words,MYBUFSZ);
//
//		// 返却するzvalの定義
//		zval *return_array;
//		MAKE_STD_ZVAL(return_array);
...
//
//		// 文字の分割
//		separated_word = strtok( words, separator );
//	    while( separated_word != NULL ){
//			add_next_index_stringl(return_array,separated_word,strlen(separated_word),1);
//		    separated_word= strtok( NULL, separator );  /* 2回目以降 */
//		}
//
//		// 配列を返却
//		*return_value = *return_array;
